...
$username_signup1 = filter_var(trim($_POST['username_signup']), FILTER_SANITIZE_STRING);
$password_signup1 = filter_var(trim($_POST['password_signup']));
$email1 = filter_var(trim($_POST['email']), FILTER_VALIDATE_EMAIL);
...
if (isset($username_signup1) && isset($password_signup1) && isset($email1) && !empty($username_signup1) && !empty($password_signup1) && !empty($email1)){
		...
		$email = $email1;
		$password = password_hash($password_signup1, PASSWORD_DEFAULT);
		$username = $username_signup1;

		$query = "select email from user where email = '".$email."' or username = '".$username."';";

		$result = $db_conn->query($query);

		if($result->num_rows > 0) {
			$error2 = "Email or Username already registered. Please try again.";
			$db_conn->close();
		} else{
			...
			$row = mysqli_fetch_row($result);
			$value = $row[0];
			$query = ("insert into customer (user_id, membership) values ('$value', FALSE);");
			$db_conn->Query($query);
			$db_conn->close();
...
<html lang="en-US" dir="ltr">
  <head>
    ...
    =============================================
    -->
    <title>Account | Local Movie Theater</title>
    <!--  
    Favicons
    =============================================
    -->
  ...
  <body data-spy="scroll" data-target=".onpage-navigation" data-offset="60">
    <main>
      ...
      <div class="main">
        <section class="module bg-dark-30" data-background="assets/images/auditorium.jpeg">
          <div class="container">
            <div class="row">
              <div class="col-sm-6 col-sm-offset-3">
                <h1 class="module-title font-alt mb-0">Login/Sign Up</h1>
              </div>
            </div>
          </div>
        </section>
          ...
          <div class="container">
            <div class="row">
              <div class="col-sm-5 col-sm-offset-1 mb-sm-40">
                ...
                <form class="form" method="post" action="loginsignup.php">
                  <div class="form-group">
                    <input class="form-control" id="username" type="text" name="username" placeholder="Username" style="text-transform: none;" required/>
                  </div>
                  <div class="form-group">
                    <input class="form-control" id="password" type="password" name="password" placeholder="Password" style="text-transform: none;" required/><?php echo htmlspecialchars($error);?>
                  </div>
                  <div class="form-group">
              ...
              <div class="col-sm-5">
                ...
                <form class="form" method="post" action="loginsignup.php">
                  <div class="form-group">
                    <input class="form-control" id="E-mail" type="email" name="email" placeholder="Email (ex. mail@example.com)" style="text-transform: none;" required/>
                  </div>
                  <div class="form-group">
                    <input class="form-control" id="username" type="text" name="username_signup" min-length="8" placeholder="Username (at least 8 characters)" style="text-transform: none;" required/>
                  </div>
                  <div class="form-group">
                    <input class="form-control" id="password_signup" type="password" name="password_signup" placeholder="Password" style="text-transform: none;" required/>
					<?php echo htmlspecialchars($error2);?>
                  </div>
                  <div class="form-group">
                    <button class="btn btn-block btn-round btn-b">Sign Up</button>
                  </div>
                </form>
              </div>
            </div>
